...
function handleStepChange(stepText: string) {
  ...
  activeSpinnerInterval = setInterval(() => {
    process.stdout.write(
      `\r\x1b[K${pc.magenta(frames[frameIndex])} ${activeStepText}`,
    );
...
function stopProgress(success = true) {
  if (activeSpinnerInterval) {
    clearInterval(activeSpinnerInterval);
    process.stdout.write(
      `\r\x1b[K${success ? pc.green("") : pc.red("")} ${activeStepText}\n`,
    );
...
function renderSummary(options: GeneratorOptions) {
  ...
  const displayValues = {
    ...
    validation:
      options.validation === "zod" ? "Zod Validation" : "None (Skipped)",
    ...
    install: options.skipInstall
      ? pc.yellow("Skipped (Auto-install off)")
      : pc.green("Completed"),
  };
  ...
  console.log(
    getLine(
      `${pc.dim("Shadcn UI:".padEnd(14))} ${pc.green(displayValues.shadcn)}`,
    ),
  );
  console.log(
    getLine(
      `${pc.dim("Resend Email:".padEnd(14))} ${pc.cyan(displayValues.email)}`,
    ),
  );
  console.log(
    getLine(`${pc.dim("Docker:".padEnd(14))} ${pc.blue(displayValues.docker)}`),
  );
  console.log(
    getLine(`${pc.dim("Installation:".padEnd(14))} ${displayValues.install}`),
  );
  console.log(getLine(""));
  console.log(lineSeparator);
  console.log(getLine(""));
  console.log(
    getLine(pc.bold(pc.green("🚀 Project scaffolded successfully!"))),
  );
  console.log(getLine(""));
  ...
  if (options.skipInstall) {
    console.log(
      getLine(
        `  ${step++}. Install dependencies: ${pc.cyan(pm === "npm" ? "npm install" : `${pm} install`)}`,
      ),
    );
  }
  ...
  if (options.db === "postgresql") {
    console.log(
      getLine(`  ${step++}. Configure ${pc.yellow("DATABASE_URL")} in .env`),
    );
    console.log(
      getLine(
        `  ${step++}. Run migrations: ${pc.cyan(pm === "npm" ? "npx prisma db push" : `${pm}x prisma db push`)}`,
      ),
    );
  }
  ...
  if (options.email === "resend") {
    console.log(
      getLine(`  ${step++}. Configure ${pc.yellow("RESEND_API_KEY")} in .env`),
    );
  }

  if (options.auth === "clerk") {
    console.log(
      getLine(`  ${step++}. Configure ${pc.yellow("CLERK keys")} in .env`),
    );
  } else if (options.auth === "auth0") {
    console.log(
      getLine(`  ${step++}. Configure ${pc.yellow("AUTH0 keys")} in .env`),
    );
  }

  console.log(
    getLine(
      `  ${step++}. Run dev server: ${pc.cyan(pm === "npm" ? "npm run dev" : `${pm} dev`)}`,
    ),
  );
  ...
  console.log(
    getLine(
      `${pc.yellow("●")}  ${pc.yellow(pc.bold("Ready for development."))}`,
    ),
  );
  console.log(
    getLine(
      pc.dim("   Check package.json to view all configured dependencies!"),
    ),
  );
  console.log(getLine(""));
  console.log(border("└" + "─".repeat(width - 2) + "┘"));
...
async function main() {
  ...
  try {
    ...
    const targetPath =
      projectLocation === "."
        ? process.cwd()
        : path.resolve(process.cwd(), projectLocation);
    const projectName =
      projectLocation === "."
    ...
    if (!isDirEmpty(targetPath)) {
      ...
      if (!args.yes) {
        proceed = await confirm({
          message:
            "Target directory is not empty. Proceed and potentially overwrite files?",
          default: false,
          theme: inquirerTheme,
        });
      }
          ...
          choices: [
            {
              ...
              name: "Better Auth",
              description:
                "Modern, secure, framework-agnostic auth (requires DB)",
            },
            {
              value: "authjs",
              name: "Auth.js",
              description: "Flexible authentication library for Next.js",
            },
            {
              value: "clerk",
              name: "Clerk",
              description: "Complete user management and auth (hosted)",
            },
            {
              value: "auth0",
              name: "Auth0",
              description:
                "Enterprise-grade authentication and authorization (hosted)",
            },
            {
              value: "skip",
              name: "Skip",
    ...
    if (!db) {
      if (auth === "better-auth") {
        ...
        if (!args.yes) {
          console.log(
            `  ${pc.cyan("ℹ")} PostgreSQL automatically selected (required by Better Auth)`,
          );
        }
      } else if (args.yes) {
          ...
          choices: [
            {
              value: "postgresql",
              name: "PostgreSQL",
              description: "PostgreSQL relational database with Prisma ORM",
            },
    ...
    if (!args.yes) {
      ...
      console.log(
        `  ${pc.dim("Review the staged changes before writing files to disk")}`,
      );
      console.log("");
      ...
      console.log(
        getReviewLine(`${pc.bold("Target Folder:")} ${pc.cyan(targetPath)}`),
      );
      console.log(
        getReviewLine(`${pc.bold("Project Name:")}  ${pc.cyan(projectName)}`),
      );
      console.log(cardBorder("├" + "─".repeat(boxWidth - 2) + "┤"));
      ...
      if (stagedFiles.length > maxShow) {
        console.log(
          getReviewLine(
            `  • ... and ${stagedFiles.length - maxShow} more files`,
          ),
        );
      }
      ...
      const prodDeps = Object.entries(staged.dependencies).map(
        ([k, v]) => `${k}@${v}`,
      );
      const devDeps = Object.entries(staged.devDependencies).map(
        ([k, v]) => `${k}@${v}`,
      );
      ...
      if (prodDeps.length > 4) {
        console.log(
          getReviewLine(
            `  • ... and ${prodDeps.length - 4} more prod dependencies`,
          ),
        );
      }
      ...
      if (devDeps.length > 4) {
        console.log(
          getReviewLine(
            `  • ... and ${devDeps.length - 4} more dev dependencies`,
          ),
        );
      }
      ...
      const action = await select({
        ...
        choices: [
          {
            value: "install",
            name: "Yes, create files and install dependencies automatically",
          },
          {
            value: "skip-install",
            name: "Yes, create files only (skip dependency installation)",
          },
